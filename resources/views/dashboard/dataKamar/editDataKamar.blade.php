...
@section('content')
    <div class="p-8">
        <div class="mb-8 anim-fade-up" style="animation-delay: 0.05s;">
            <h1 class="font-display text-3xl font-bold text-white tracking-tight">Halaman Form Edit Data Kamar</h1>
        </div>

        <div class="lg:col-span-2 bg-slate-800 rounded-2xl p-6 animate-fade-up" style="animation-delay:0.4s">
            <form action="/dashboard/kamar/edit_data_kamar/{{ $room->id }}" method="POST">
                @csrf
                <div class="space-y-12">
                    <div class="border-b border-white/10 pb-12">
                        <div class="mt-2 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                            <div class="sm:col-span-4">
                                <label for="kost_id" class="block text-sm/6 font-medium text-white">Kosts <span
                                        class="text-red-500">*</span></label>
                                <div class="mt-2 grid grid-cols-1">
                                    <select id="kost_id" name="kost_id"
                                        ...
                                        <option value="">-- Pilih Kosts --</option>
                                        @foreach ($kosts as $item)
                                            <option value="{{ $item->id }}"
                                                {{ old('kost_id', $room->kost_id) == $item->id ? 'selected' : '' }}>
                                                {{ $item->nama_kosts }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="sm:col-span-4">
                                <label for="nomor_kamar" class="block text-sm/6 font-medium text-white">Nomor Kamar <span
                                        class="text-red-500">*</span></label>
                                <div class="mt-2">
                                    <div
                                        ...
                                        <input id="nomor_kamar" type="number" name="nomor_kamar"
                            ...
                            <div class="sm:col-span-4">
                                <label for="harga_bulanan" class="block text-sm/6 font-medium text-white">Harga Bulanan
                                    <span class="text-red-500">*</span></label>
                                <div class="mt-2">
                                    <div
                                        class="flex items-center rounded-md bg-white/5 pl-3 outline-1 -outline-offset-1 outline-white/10 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-500">
                                        Rp. <input id="harga_bulanan" type="number" name="harga_bulanan"
                                            placeholder="masukan harga bulanan"
                                            value="{{ old('harga_bulanan', $room->harga_bulanan) }}"
                                            class="block min-w-0 grow bg-transparent py-1.5 pr-3 pl-1 text-base text-white placeholder:text-gray-500 focus:outline-none sm:text-sm/6" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="sm:col-span-4">
                                ...
                                <div class="mt-2 grid grid-cols-1">
                                    <select id="status" name="status"
                                        class="col-start-1 row-start-1 w-full appearance-none rounded-md bg-white/5 py-1.5 pr-8 pl-3 text-base text-white outline-1 -outline-offset-1 outline-white/10 *:bg-gray-800 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6">
                                        <option value="kosong"
                                            {{ old('kosong', $room->kosong) == 'kosong' ? 'selected' : '' }}>
                                            kosong
                                        </option>
                                        <option value="terisi"
                                            {{ old('terisi', $room->terisi) == 'terisi' ? 'selected' : '' }}>
                                            terisi
                                        </option>
                                        <option value="maintenance"
                                            {{ old('maintenance', $room->maintenance) == 'maintenance' ? 'selected' : '' }}>
                                            maintenance</option>
                                    </select>
                                    <svg viewBox="0 0 16 16" fill="currentColor" data-slot="icon" aria-hidden="true"
                                        class="pointer-events-none col-start-1 row-start-1 mr-2 size-5 self-center justify-self-end text-gray-400 sm:size-4">
